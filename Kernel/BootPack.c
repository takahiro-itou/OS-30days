...
void HariMain(void)
{
    ...
    for (i = 0; i < 3; ++ i) {
        ...
        snprintf(s, sizeof(s) - 1, "task_b%d", i);
...
void make_textbox8(struct SHEET *sht, int x0, int y0, int sx, int sy, int c)
{
    ...
    boxfill8(sht->buf, sht->bxsize, COL8_C6C6C6, x0+1, y0-2, x1+1, y1+1);
...
void task_b_main(struct SHEET *sht_win_b)
{
    ...
    for (;;) {
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            io_sti();
            if (i == 100) {
                snprintf(s, sizeof(s) - 1, "%12d", count - count0);
                putfonts8_asc_sht(sht_win_b, 24, 28, COL8_000000,
                                  COL8_C6C6C6, s, 12);
                count0 = count;
                timer_settime(timer_1s, 100);
            }
